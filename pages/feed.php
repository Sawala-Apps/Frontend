...
<body class="bg-white bg-[linear-gradient(to_right,#80808033_1px,transparent_1px),linear-gradient(to_bottom,#80808033_1px,transparent_1px)] bg-[size:70px_70px] font-sans items-center flex flex-col">
    ...
    function createPostElement(post) {
        ...
        <div id="imagePost" class="mt-4 flex flex-col items-center">
            ...
            <div class="mt-1 flex flex-row w-[100%]">
               ...
               <button id="shareButton" class="btn-brutal basis-1/3 btn flex items-center justify-center space-x-2 px-4 py-2 bg-[#88AAEE] text-black font-bold rounded-md shadow-join hover:bg-yellow-600 border-brutal">
    ...
    const shareBtn = postContainer.querySelector("#shareButton");
    shareBtn.addEventListener('click', function (event) {
        event.preventDefault(); 
        event.stopPropagation();
        const postUrl = `https://nova-agustina.my.id/22cns/John/Sawala/Frontend/pages/detail.php?postid=${post.postid}`
        navigator.clipboard.writeText(postUrl)
        .then(() => {
            toastr.success("Link berhasil disalin ke clipboard!", "Sukses", {
                    closeButton: true,
                    progressBar: true,
        ...
        .catch(err => {
            toastr.error("Gagal menyalin ke clipboard", "Error", {
                    closeButton: true,
                    progressBar: true,
                    positionClass: "toast-top-right",
                    timeOut: 3000
